...
- Accurate font-derived grid resize and retained Metal presentation. CoreText rasterizes only rows whose core-provided visual hashes changed, preserving native shaping, fallback, colour glyphs, per-cell colours, complete underline variants and terminal cursors; Metal blits retained rows during viewport scrolling and composites the result without an idle display loop.
...
- Bounded Kitty graphics snapshots with generation-keyed, transfer-once RGBA image caching, source cropping and Retina-aware placement.
