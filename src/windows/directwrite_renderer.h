...
typedef struct ZigonautGlyphAtlasPixelsTest {
    ...
    uint64_t first_uploads;
    uint64_t first_rasterizations;
    uint64_t second_uploads;
    uint64_t second_rasterizations;
...
// Draws a procedural cell from an A8 coverage mask. This is deliberately
// separate from the UTF-16/glyph-index API: codepoint identifies the cached
// builtin and mask contains width-by-height coverage generated by Zig. The
// engine copies the mask before returning when row rendering is active.
HRESULT zigonaut_text_engine_draw_builtin_cell(
    ZigonautTextEngine* engine, uint32_t codepoint,
    const uint8_t* mask, uint32_t mask_width, uint32_t mask_height, uint32_t mask_stride,
