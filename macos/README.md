...
- Desktop notifications are queued (32, 4096 combined UTF-8 bytes), delivered only while no Zigonaut window is key, and use unique identifiers. Permission is requested once by macOS.
- Persisted live font size, padding, colour scheme, terminal background opacity, and guarded OSC 52 clipboard settings. Clipboard writes default off, are bounded and only accepted immediately when explicitly enabled; the terminal callback cannot defer a reply for later confirmation.
