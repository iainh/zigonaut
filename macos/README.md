...
- Configurable executable absolute shell path for new panes (invalid values visibly fall back to `/bin/zsh`). Automatic prompt and working-directory integration is enabled for directly launched zsh 5.1+, fish 2.7+, and non-system Bash 4.4+. It can be disabled globally and applies only to new terminals. Zigonaut doesn't edit startup files; `/bin/bash`, unsupported shells, and `zsh -f` remain unchanged.
...
- Adjustable line height (75%–200%) under Settings > Appearance > Typography, independent of font size. The default is 100%; tighter spacing can clip tall glyphs.
