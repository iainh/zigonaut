...
- Configurable executable absolute shell path for new panes (invalid values visibly fall back to `/bin/zsh`). Automatic prompt and working-directory integration is enabled for directly launched zsh 5.1 or later, can be disabled globally, and applies only to new terminals. Zigonaut doesn't edit startup files; unsupported shells and `zsh -f` remain unchanged.
