...
const Application = struct {
    ...
    fn reloadViews(self: *Application, font: win.HFONT, settings: config.Config, size: u16, dpi: u32) !void {
        ...
        for (self.views.items, 0..) |entry, index| {
            prepared[index] = try entry.view.prepareReload(font, settings.font_family, settings.symbol_fallback_family, size, @intFromEnum(settings.font_weight), @intFromEnum(settings.intense_font_weight), @intFromEnum(settings.text_antialiasing), dpi);
            count += 1;
        }
